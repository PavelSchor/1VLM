...
from flight.fcs2 import FlightControlSystem2, ControlDeflection, CompoundControlDeflection, ControlInputs
from flight.fdm1 import FlightModel1
from flight.stabilityDerivatives import Derivative, TableDerivatives
...
vtu.ctrMask=np.zeros(len(vtu.panels),dtype=bool)
lmVtu=list(np.array([list(range(i*10+7,i*10+10)) for i in range(1,9)],dtype=int).flatten())
vtu.ctrMask[lmVtu]=True
...
vtuAx1=np.array([7.35,0.,-2.5])
vtuAx2=np.array([7.35,0.,2.5])
vtu.AX_O=vtuAx1
vtu.AX=vtuAx2-vtuAx1



fcs=FlightControlSystem2()
pr1.fcs=fcs

dvr1=ControlDeflection()
dvr1.setRng(np.deg2rad([-20,20]))

dvr1.inputs=fcs.inputs
dvr1.cmds.append('rudder')


dhl1=ControlDeflection()
dhl1.setRng(np.deg2rad([-30,30]))

dhl2=ControlDeflection()
dhl2.setRng(np.deg2rad([-30,30]))


cdhl=CompoundControlDeflection()
(cdhl.dmin,cdhl.dmax)= np.deg2rad([-60,60])

cdhl.surfaces.append(dhl1)
cdhl.surfaces.append(dhl2)

cdhl.inputs=fcs.inputs
cdhl.cmds.append('elevator')
cdhl.cmds.append('aileron')


dhr1=ControlDeflection()
dhr1.setRng(np.deg2rad([-30,30]))
dhr2=ControlDeflection()
dhr2.setRng(np.deg2rad([30,-30]))

cdhr=CompoundControlDeflection()
(cdhr.dmin,cdhr.dmax)= np.deg2rad([-60,60])

cdhr.surfaces.append(dhr1)
cdhr.surfaces.append(dhr2)

cdhr.inputs=fcs.inputs
cdhr.cmds.append('elevator')
cdhr.cmds.append('aileron')



fcs.groupSurfaces['rudder']=vtu
fcs.groupSurfaces['htp_l']=htu_l
fcs.groupSurfaces['htp_r']=htu_r

fcs.groupDeflections['rudder']=dvr1
fcs.groupDeflections['htp_l']=cdhl
fcs.groupDeflections['htp_r']=cdhr
...
pr1.setVelocity([30,0,0])


d_a=Derivative()
d_a.setMain(pr1)
d_a.setObject(pr1.alpha[0:1])
d_a.setIncrement(np.deg2rad(10))

d_b=Derivative()
d_b.setMain(pr1)
d_b.setObject(pr1.beta[0:1])
d_b.setIncrement(np.deg2rad(10))


d_p=Derivative()
d_p.setMain(pr1)
d_p.setObject(pr1.dom1.OMEGA[0:1])
d_p.setIncrement(1.)

d_q=Derivative()
d_q.setMain(pr1)
d_q.setObject(pr1.dom1.OMEGA[1:2])
d_q.setIncrement(1.)

d_r=Derivative()
d_r.setMain(pr1)
d_r.setObject(pr1.dom1.OMEGA[2:3])
d_r.setIncrement(1.)

d_roll=Derivative()
d_roll.setMain(pr1)
d_roll.setObject(pr1.fcs.inputs.aileron[0:1])
d_roll.setIncrement(1.)

d_pitch=Derivative()
d_pitch.setMain(pr1)
d_pitch.setObject(pr1.fcs.inputs.elevator[0:1])
d_pitch.setIncrement(1.)

d_yaw=Derivative()
d_yaw.setMain(pr1)
d_yaw.setObject(pr1.fcs.inputs.rudder[0:1])
d_yaw.setIncrement(1.)
...
t=TableDerivatives()
t.constCol=np.array([qs,qs,qsc,qs,qs,qs,qsb,qsc,qsb])
t.addDerivative(d_a,'alpha')
t.addDerivative(d_b,'beta')
t.addDerivative(d_p,'p')
t.addDerivative(d_q,'q')
t.addDerivative(d_r,'r')
t.addDerivative(d_roll,'roll cmd')
t.addDerivative(d_pitch,'pitch cmd')
t.addDerivative(d_yaw,'yaw cmd')
t.compute()
df=t.getDataFrame()

df.to_excel('stabilityDerivatives_example7.xlsx')
