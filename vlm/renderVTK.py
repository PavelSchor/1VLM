...
class vtkTimerCallback():
	def __init__(self):
		self.pos=np.array([16e3,9.4e3,0.6e3],dtype=np.float)
		self.focalVect=np.array([10,0,0],dtype=np.float)
		self.focal=np.zeros(3,dtype=np.float)
		self.dv=np.array([0.9,0,0],dtype=np.float)
		pass
		#self.timer_count = 0
	...
	def execute(self, obj, event):
		...
		self.focal=self.pos+self.focalVect
		self.camera.SetPosition(self.pos[0],self.pos[1],self.pos[2])
		self.camera.SetFocalPoint(self.focal[0],self.focal[1],self.focal[2]) 
		self.camera.SetViewUp(0.0,0.0,1.0)

class vtkTimerFdmCallback():
	def __init__(self):
		self.pos=np.zeros(3,dtype=np.float)
		self.focal=np.zeros(3,dtype=np.float)
		self.viewUp=np.zeros(3,dtype=np.float)
		pass
	...
	def execute(self, obj, event):
		...
		self.pos=self.fdm.getCameraPos()
		self.focal=self.fdm.getCameraFocal()
		self.viewUp=self.fdm.getCameraViewUp()
		self.camera.SetPosition(self.pos[0],self.pos[1],self.pos[2])
		self.camera.SetFocalPoint(self.focal[0],self.focal[1],self.focal[2]) 
		self.camera.SetViewUp(self.viewUp[0],self.viewUp[1],self.viewUp[2],)
		vm=3.6*np.linalg.norm(self.fdm.V)
		alpha=np.rad2deg(np.arctan2(self.fdm.dom1.VEL[2],self.fdm.dom1.VEL[0]))
		beta=np.rad2deg(np.arctan2(self.fdm.dom1.VEL[1],self.fdm.dom1.VEL[0]))
		#print alpha, beta
		om=self.fdm.OMEGA
		omm=self.fdm.dom1.OMEGA
		self.captionTextActor.SetInput("V: {:f}  a: {:f}  b: {:f} OMEGA:[ {:f} {:f} {:f}  ] {:f}".format(vm,alpha,beta,om[0],om[1],om[2],omm[0]))
		if np.any(np.rad2deg(self.fdm.dom1.ALPHA) >100):
			iren.DestroyTimer()
			iren.TerminateApp()
		iren.GetRenderWindow().Render()
...
class PanelViewer3D(object):
	...
	def __init__(self):
		...
		self.showAxisSystems=False
		self.showCompass=True
		self.showAxes=True
		self.showScalarBar=False
		self.showCaption=False
		self.showXYPlane=False
		self.showYZPlane=False
		self.showZXPlane=False
		self.xlabel='x'
		self.ylabel='y'
		self.zlabel='z'
		self.xrange=None
		...
		self.renWin.SetWindowName("VTK ")
		self.iren = vtk.vtkRenderWindowInteractor();
		self.iren.SetRenderWindow(self.renWin);
		self.ren.GradientBackgroundOn();
		self.ren.SetBackground(1,1,1);
		self.ren.SetBackground2(0.3,0.3,1);
		light = vtk.vtkLight()
	...
	def show(self,motionModel=None):
		if motionModel is None:
			style1 = vtk.vtkInteractorStyleTrackballCamera()
			self.iren.SetInteractorStyle(style1)
		self.ren.ResetCamera()
		
		self.iren.Initialize();
		if motionModel is not None:
			self.callback = vtkTimerFdmCallback()
			self.callback.fdm=motionModel
			self.callback.camera=self.camera
			self.callback.captionTextActor=self.captionTextActor
			self.iren.AddObserver('TimerEvent', self.callback.execute)
			self.iren.CreateRepeatingTimer(30)
	...
	def loadMesh(self,fname,texture=None):
...
#		mapper.InterpolateScalarsBeforeMappingOn()
	...
	def addWidgets(self,opt=None):
		...
		self.captionTextActor = vtk.vtkTextActor()
		self.captionTextActor.GetTextProperty().SetFontSize ( 26 )
		self.captionTextActor.SetPosition2( 100, 100 )
		self.captionTextActor.SetInput(r'FDM: '+str(self.caption))#+r' Scaling: '+str(self.scalingFactor))
		self.captionTextActor.GetTextProperty().SetColor ( 0.0,0.0,0.0 )
		...
		if self.showCaption:
			self.ren.AddActor2D( self.captionTextActor )
