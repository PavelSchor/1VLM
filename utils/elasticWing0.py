...
class ElasticWing1(object):
	...
	def addSubSection(self,section,nodes,spar,ofsCC=0,integration='12',j1=0,j2=2,oi=0,i1=20,i2=40,oVector=np.array([0,0,1.]),reverse=False):
		...
		for i in kk:
			...
			for s in pgs[i].getPanelList():
				...
				if not pan.isWake:
					c1.addSurfacePoint(pan.P1)
					c2.addSurfacePoint(pan.P2)
				if pan.isLast:
					c1.addSurfacePoint(pan.P4)
					c2.addSurfacePoint(pan.P3)
	...
	def connectNodes(self,nodes,start,stop,ofsN=0,ofsI=0): 
		...
		for i in range(start,stop):
			if interactors[i].integration == '12':
				...
				interactors[i].ids[1]=i+ofsN+1		
			if interactors[i].integration == '21':
				nodes[i+ofsN]=interactors[i].section[1].orig.copy()
				nodes[i+ofsN+1]=interactors[i].section[0].orig.copy()
				interactors[i].node[0]=nodes[i+ofsN]
				interactors[i].node[1]=nodes[i+ofsN+1]
				interactors[i].ids[0]=i+ofsN
