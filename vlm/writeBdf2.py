...
PARAM   PRTMAXIM YES''')
	...
	for e in elem:
		...
		cn=elem[e]['conn']
		if etype=='CQUAD4':
			f.write(print_card_8(['CQUAD4', eid, pid ,cn[0],cn[1],cn[2],cn[3]]))

		if etype=='CTRIA3':
			f.write(print_card_8(['CTRIA3', eid, pid ,cn[0],cn[1],cn[2]]))


	for i in range(0,grid.shape[0]):
		###f.write('{:8s}{:16d}{:16s}{:16.8f}{:16.8f}{:8s}\n{:8s}{:16.8f}{:16s}{:16s}{:16s}{:8s}\n'.format('GRID*', i+1, '',sc*grid[i,0],sc*grid[i,1],'','*',sc*grid[i,2],'','','',''))
		f.write(print_card_double(['GRID', i+1, '',sc*grid[i,0],sc*grid[i,1],sc*grid[i,2]]))
